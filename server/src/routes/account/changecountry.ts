import { Router, Request, Response, NextFunction } from "express"
import jwt from "jsonwebtoken"
import User from "../../../models/User"
import dotenv from "dotenv"
dotenv.config()
const router = Router()

function isBlank(str: string) {
    return !str || /^\s*$/.test(str)
}

router.post("/", async (req: Request, res: Response, next: NextFunction) => {
    const country: string = req.body.country?.value?.toLowerCase()
    const token: string = req.body.token

    if (isBlank(country)) return res.status(403).send("Invalid country")

    try {
        const temporaryUser: any = jwt.verify(token, process.env.JWT_SECRET as string)
        const verifyUser = await User.findOne({
            _id: temporaryUser.id,
        })
        if (!verifyUser) return res.status(400).send("Not authenticated")

        await User.findByIdAndUpdate(verifyUser.id, {
            $set: {
                country,
            },
        })
        res.status(200).send("Successfully changed country")
    } catch {
        return res.status(400).send("Something went wrong")
    }
})

export default router
